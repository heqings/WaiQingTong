...
+(CustomBadge*)getBadgeWithView:(UIView*)view
{
    //
    CustomBadge* temp=nil;
    for (UIView* v in view.subviews)
    {
        if([v isKindOfClass:[CustomBadge class]])
        {
          temp = (CustomBadge*)v;
            break;
        }
...
+(CustomBadge*) addBadgeIntoView:(UIView*)view
{
    if(view == nil)
        return nil;
    CustomBadge* cb =  [CustomBadge customBadgeWithString:@"" 
                                      ...
                                      withBadgeFrameColor:[UIColor whiteColor] 
                                                withScale:1.0
                                              withShining:YES];
    cb.frame = CGRectMake(view.bounds.size.width-12.5, -12.5, 25, 25);
...
+(BOOL)changeBadgeTextWithView:(UIView*)view  changeText:(NSString*)text
{
    ...
    for (UIView* v in view.subviews)
    {
        if([v isKindOfClass:[CustomBadge class]])
        {
            CustomBadge* c = (CustomBadge*)v;
            [c autoBadgeSizeWithString:text];
            bChange=YES;
        }
    }
    return bChange;
...
+(BOOL)setBadgeNoVisibleWithView:(UIView*)view
{
    bool bChange=NO;
    for (UIView* v in view.subviews)
    {
